...
vmod_reset_offset(VRT_CTX, VCL_ENUM type){
	vmodreq_seek_reset(ctx->req, vmod_convtype(type));
}

VCL_VOID
vmod_post_seek_reset(VRT_CTX){
	vmodreq_seek_reset(ctx->req, POST);
}
VCL_VOID
vmod_get_seek_reset(VRT_CTX){
	vmodreq_seek_reset(ctx->req, GET);
}
VCL_VOID
vmod_cookie_seek_reset(VRT_CTX){
	vmodreq_seek_reset(ctx->req, COOKIE);
}

///////////////////////////////////////////////////////////////
//キーのオフセットを移動＋移動後の値取得
const char *vmod_next_key(VRT_CTX, VCL_ENUM type){
	return vmodreq_seek(ctx->req, vmod_convtype(type));
}

const char* vmod_get_read_keylist(VRT_CTX){
	return vmodreq_seek(ctx->req, GET);
}
const char* vmod_post_read_keylist(VRT_CTX){
	return vmodreq_seek(ctx->req, POST);
}
const char* vmod_cookie_read_keylist(VRT_CTX){
	return vmodreq_seek(ctx->req, COOKIE);
}
...
vmod_next_offset(VRT_CTX, const char *type){
	vmodreq_seek(ctx->req, vmod_convtype(type));
...
const char* vmod_current_key(VRT_CTX, VCL_ENUM type){
	return vmod_read_cur(ctx->req, vmod_convtype(type));
...
unsigned vmod_iterate(VRT_CTX, VCL_ENUM type, const char* p){
	return vmod_read_iterate(ctx->req, p,vmod_convtype(type));
}
...
{
	unsigned ret = 0;
	enum gethdr_e where = vmod_convhdrtype(ctx->req, type, &ret);
	if(ret){
		//headerの値を作る必要がある
		char tmp[256];
		gen_hdrtxt(header, tmp, 256);
		char * val = VRT_GetHdr(ctx->req,  where, tmp);
		if(val){
			return strlen(VRT_GetHdr(ctx->req,  where, tmp));
		}else{
			return 0;
		}
	}else{
		return vmodreq_headersize(ctx->req, vmod_convtype(type) ,header);
	}
}
...
const char *vmod_param(VRT_CTX, const char *type ,const char *header){
	...
	if(ret){
		...
		char tmp[256];
		gen_hdrtxt(header, tmp, 256);
		return VRT_GetHdr(ctx->req,  where, tmp);
	}else{
		return vmodreq_header(ctx->req, vmod_convtype(type) ,header);
	}
}

const char *vmod_post_header(VRT_CTX, const char *header){
	return vmodreq_header(ctx->req, POST,header);
}

const char *vmod_get_header(VRT_CTX, const char *header){
	return vmodreq_header(ctx->req, GET,header);
}

const char *vmod_cookie_header(VRT_CTX, const char *header){
	return vmodreq_header(ctx->req, COOKIE,header);
}
...
const char* vmod_body(VRT_CTX, const char *type){
	if(!vmodreq_get_raw(sp)){
		VRT_panic(ctx->req, "please write \"parsereq.init();\" to 1st line in vcl_recv.",vrt_magic_string_end);
	}
...
const char* vmod_post_body(VRT_CTX){
	return vmod_body(ctx->req,  "post");
}

const char* vmod_get_body(VRT_CTX){
	return vmod_body(ctx->req,  "get");
}

const char* vmod_cookie_body(VRT_CTX){
	return vmod_body(ctx->req,  "cookie");
}
///////////////////////////////////////////////////////////////
//初期化、デバッグなどシステム系
VCL_VOID
